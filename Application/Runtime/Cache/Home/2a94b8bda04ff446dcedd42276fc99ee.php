        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            <div class="am-btn-group am-btn-group-xs">
              <a href="<?php echo U('Index/add_report');?>"><button type="button" class="am-btn am-btn-default"><span class="am-icon-plus"></span> 创建报告</button></a>
            </div>
          </div>
        </div>
      ...
      <div class="am-g">
        <div class="am-u-sm-12">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tbody>
            <?php if(is_array($report_list)): foreach($report_list as $key=>$vo): ?><tr>
                <td><?php echo ($vo["id"]); ?></td>
                <td><?php echo ($vo["poject_name"]); ?></td>
                <td><?php echo ($vo["poject_start_time"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($vo["poject_end_time"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($vo["add_time"]); ?></td>
                <td>
                  <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <a href="<?php echo U('Index/edit_report_info',array('id'=>$vo['id']));?>"  class="am-btn am-btn-default am-btn-xs am-hide-sm-only"><span class="am-icon-copy"></span> 编辑</a>
                      <button type="button" id="<?php echo ($vo["id"]); ?>" onclick="confirm_del_report(this)" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"><span class="am-icon-trash-o"></span> 删除</button>
                      <a href="<?php echo U('Download/download',array('id'=>$vo['id']));?>"  class="am-btn am-btn-default am-btn-xs am-hide-sm-only"><span class="am-icon-download"></span> 下载报告</a>
                      <a href="<?php echo U('Index/system_list',array('id'=>$vo['id']));?>"  class="am-btn am-btn-default am-btn-xs am-hide-sm-only"><span class="am-icon-copy"></span> 目标列表</a>
...
<script>
  ...
  function confirm_del_report(_this)
  {
    if (confirm("确定要删除该报告吗?(确定后将删除关联该报告的一切信息包括图片),该操作不可逆!")) {
      ...
      $.ajax({
        url:"<?php echo U('Index/del_report');?>",
        type:"post",
        data:'id='+id,
